  ...
  methods: {
    ...
    handleSearch() {
      ...
      Object.keys(this.filtered_data).forEach((item) => {
        ...
        this.filtered_data[item] == 'status_true'
          ? (this.filtered_data.status = 1)
          : this.filtered_data[item] == 'status_error'
          ? (this.filtered_data.status = 0)
          : this.filtered_data[item] == 'wifi' ||
            this.filtered_data[item] == 'eth' ||
            this.filtered_data[item] == 'm2m'
          ? (this.filtered_data.connection = this.filtered_data[item])
          : this.filtered_data[item] == 'battery_alarm'
          ? (this.filtered_data.battery = 9)
          : this.filtered_data[item] == 'plug_in'
          ? (this.filtered_data.electric_connection = true)
          : ''
        //   this.filtered_data[item] == 'plug_in'
        //   ?  this.filtered_data.battery_active = true
        //   :
      })
      //   if (this.filtered_data.state != undefined)
      //     state = this.filtered_data.state.join()
      delete this.filtered_data.state
